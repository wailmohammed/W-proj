  ...
  return (
    <div className="max-w-6xl mx-auto animate-fade-in space-y-8 pb-20 px-4 md:px-0">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex bg-slate-100 dark:bg-slate-900 p-1 rounded-xl border border-slate-200 dark:border-slate-800 w-fit">
              {['1M', '6M', 'YTD', '1Y', 'ALL'].map(tf => (
                  <button key={tf} onClick={() => setTimeframe(tf as any)} className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-all ${timeframe === tf ? 'bg-white dark:bg-slate-800 text-brand-600 dark:text-brand-400 shadow-sm' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}>{tf}</button>
              ))}
          </div>
      </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
              <div>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                    <TrendingUp className="w-5 h-5 text-brand-500" /> Index Benchmarking
                  </h3>
                  <p className="text-sm text-slate-500">How you compare to major market indices</p>
              </div>
              <div className="flex flex-wrap gap-2">
                  {(['sp500', 'nasdaq'] as const).map(b => (
                      <button key={b} onClick={() => setBenchmark(b)} className={`px-4 py-2 rounded-xl text-[10px] font-black border transition-all ${benchmark === b ? 'bg-brand-600 text-white border-brand-600 shadow-lg shadow-brand-600/20' : 'bg-white dark:bg-slate-950 text-slate-500 border-slate-200 dark:border-slate-800 hover:border-slate-400'}`}>
          ...
          <div className="h-[300px] md:h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={comparisonData}>
                      ...
                      <RechartsTooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '12px', fontSize: '12px' }} itemStyle={{ fontWeight: 'bold' }} formatter={(val: number) => [`${val}%`]} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2"><Target className="w-5 h-5 text-emerald-500" /> Sector Weighting</h3>
              <div className="space-y-4">
                  {holdings.reduce((acc, h) => {
                      const existing = acc.find(x => x.name === h.sector);
                      const weight = (h.shares * h.currentPrice) / activePortfolio.totalValue * 100;
                      if (existing) existing.value += weight; else acc.push({ name: h.sector, value: weight });
                      return acc;
                  ...
                  .sort((a, b) => b.value - a.value).map((s, i) => (
                      <div key={s.name} className="space-y-1.5">
                          ...
                          <div className="h-2 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                              <div className="h-full bg-brand-500 transition-all duration-1000" style={{ width: `${s.value}%`, backgroundColor: COLORS[i % COLORS.length] }}></div>
                          </div>
                      </div>
                  ))}
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2"><ShieldCheck className="w-5 h-5 text-brand-500" /> Snowflake Audit</h3>

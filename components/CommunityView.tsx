...
import { MessageSquare, Heart, Share2, TrendingUp, Users, Zap, ArrowRight, ArrowUpRight, ArrowDownRight, Sparkles } from 'lucide-react';
...
const CommunityView: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto animate-fade-in space-y-8 pb-20">
      ...
      {/* Featured Insights Carousel (Blossom Style) */}
      <section>
          <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                  <Sparkles className="w-5 h-5 text-brand-500" /> Daily Insights
              </h2>
              <button className="text-sm text-brand-600 dark:text-brand-400 font-medium hover:underline">View All</button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="relative h-48 rounded-2xl p-6 flex flex-col justify-end overflow-hidden group cursor-pointer">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm">
                <div className="flex gap-4">
                    <div className="w-10 h-10 rounded-full bg-brand-600 flex items-center justify-center text-white font-bold shadow-lg shadow-brand-600/20">YO</div>
                    <input 
                        type="text" 
                ...
                <div className="flex justify-between items-center mt-3 px-1">
                    ...
                    <button className="px-5 py-2 bg-brand-600 hover:bg-brand-500 text-white rounded-xl text-sm font-bold transition-all shadow-lg shadow-brand-600/20 hover:shadow-brand-600/40">
                        Post
                    </button>
                </div>
            </div>
                    ...
                    <div key={post.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 hover:border-brand-200 dark:hover:border-slate-700 transition-all shadow-sm">
                        ...
                        <p className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed mb-4 whitespace-pre-line">
                            {post.content.split(' ').map((word, i) => 
                                ...
                                <span key={i} className="text-brand-600 dark:text-brand-400 font-bold cursor-pointer hover:underline">{word} </span> : 
                                word + ' '
                            )}
                        </p>
                        ...
                        {post.tickers && post.tickers.length > 0 && (
                            <div className="flex flex-col gap-2 mb-4">
                                {post.tickers.map(ticker => {
                                    ...
                                    return (
                                        <div key={ticker} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 group cursor-pointer hover:border-brand-300 dark:hover:border-slate-600 transition-colors">
                                            <div className="flex items-center gap-3">
                                                {asset.logoUrl ? (
                        ...
                        <div className="flex items-center gap-6 pt-4 border-t border-slate-100 dark:border-slate-800/50">
                            <button className="flex items-center gap-2 text-slate-500 hover:text-red-500 transition-colors text-sm group">
                                <Heart className="w-5 h-5 group-hover:fill-red-500" /> {post.likes}
                            </button>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
                ...
                <div className="space-y-2">
                    {[
                        ...
                        <div key={item.s} className="flex items-center justify-between p-2 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl cursor-pointer transition-colors group">
                            <div className="flex items-center gap-3">
                ...
                <button className="w-full mt-4 py-2.5 text-sm text-brand-600 dark:text-brand-400 font-bold bg-brand-50 dark:bg-brand-900/20 rounded-xl hover:bg-brand-100 dark:hover:bg-brand-900/30 transition-colors flex items-center justify-center gap-2">

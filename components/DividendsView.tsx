...
// Sustainability visual cues: Green < 60%, Yellow 60-90%, Red > 90%
                  ...
                  <div className="flex items-center gap-4">
                      ...
                      <div>
                          <div className="flex items-center gap-2">
                              ...
                              {isCut && <span className="text-[10px] font-bold bg-red-500 text-white px-1.5 rounded uppercase">Suspended</span>}
                          </div>
                          <div className="text-xs text-slate-500 truncate max-w-[140px] md:max-w-none">Yield: {payment.dividendYield}% • Payout: {payment.payoutRatio}%</div>
                      </div>
                  </div>
              ...
              <div className="flex items-center justify-between md:justify-end gap-2 md:gap-6 w-full md:flex-1 pt-2 md:pt-0 border-t md:border-t-0 border-slate-100 dark:border-slate-800/50 md:border-0">
                  <div className="flex items-center gap-4 overflow-x-auto no-scrollbar">
                      <div className="text-center min-w-[70px]">
                          <div className="text-[9px] text-slate-500 uppercase mb-1">Payout Ratio</div>
...
const DividendsView: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'date' | 'amount' | 'yield' | 'payout' | 'safety'>('date');
  ...
  // Stress-test: cut dividends below safety 60
  ...
  const sortedDividends = useMemo(() => {
      ...
      if (sortBy === 'payout') return data.sort((a, b) => b.payoutRatio - a.payoutRatio);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className={`lg:col-span-2 border rounded-2xl p-6 transition-all ${recessionMode ? 'bg-red-50 dark:bg-red-950/10 border-red-200' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800'}`}>
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h3 className={`text-lg font-bold flex items-center gap-2 ${recessionMode ? 'text-red-600' : 'text-slate-900 dark:text-white'}`}>
                            ...
                            {recessionMode ? 'Recession Active' : 'Income Stress Test'}
                        </h3>
                        <p className="text-xs text-slate-500 mt-1">Stress-test your income against risky payouts.</p>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800">
                        <div className="text-[10px] text-slate-500 uppercase font-bold mb-1">Yield on Cost</div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                    <Clock className="w-5 h-5 text-brand-500" /> Projections
                </h3>
                <div className="h-32 mb-4">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={projectionData}>
                            <Bar dataKey="income" fill="#6366f1" radius={[2, 2, 0, 0]} />
                            <RechartsTooltip cursor={false} contentStyle={{ backgroundColor: '#0f172a', borderRadius: '8px', border: 'none' }} />
                        </BarChart>
                    </ResponsiveContainer>
                </div>
                <div className="space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <input type="range" min="0" max="15" value={projectedCagr} onChange={(e) => setProjectedCagr(parseInt(e.target.value))} className="flex-1 h-1.5 bg-slate-200 rounded-full appearance-none accent-brand-500" />
                        <span className="text-xs font-bold text-slate-500 w-8">{projectedCagr}%</span>
                    </div>
                    <button onClick={() => setReinvestMode(!reinvestMode)} className={`w-full py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-2 border transition-all ${reinvestMode ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-500' : 'bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-400'}`}>
                        <RefreshCw className={`w-3 h-3 ${reinvestMode ? 'animate-spin-slow' : ''}`} /> DRIP: {reinvestMode ? 'Active' : 'Disabled'}
                    </button>
                </div>
            </div>
        </div>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        <ListFilter className="w-5 h-5 text-brand-500" /> Payment Schedule
                    </h3>
                    <select value={sortBy} onChange={(e) => setSortBy(e.target.value as any)} className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-1.5 text-xs font-bold focus:border-brand-500 outline-none">
                        ...
                        <option value="payout">Sort by Payout Ratio</option>
                        ...
                        <div key={idx} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                            ...
                            <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
                                {month.payments.map(p => {
                                    const isCut = recessionMode && p.safetyScore < 60;
                                    return <DividendRow key={p.id} payment={p} isCut={isCut} />;
                                })}
                            </div>
                        </div>
                    )) : (
